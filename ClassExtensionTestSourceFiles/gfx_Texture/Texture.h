...
public:
    ...
    */
    ...
    public:
        ...
        Visualization(Channels c = RGB, float g = 1.0f, float mn = 0.0f, float mx = 1.0f, bool mod1 = false);
...
public:
    ...
    inline int numMipMapLevels() {
        if (! hasMipMaps()) {
            ...
            const int maxRes = std::max(m_width, std::max(m_height, m_depth));
            const int numMipMaps = int(log2(float(maxRes))) + 1;
            return numMipMaps;
        }
    }
    ...
    */
    [[deprecated]] bool copyInto(shared_ptr<Texture>& dest, CubeFace cf = CubeFace::POS_X, int mipLevel = 0, RenderDevice* rd = NULL) const;
    ...
    */
    ...
    /**
     ...
     \deprecated
     */
    [[deprecated]] void copyFromScreen(const Rect2D& rect, const ImageFormat* fmt = NULL);
	...
	*/
	...
	//shared_ptr<GLPixelTransferBuffer> toPixelTransferBuffer2(const ImageFormat* outFormat = ImageFormat::AUTO(), int mipLevel = 0, CubeFace face = CubeFace::POS_X) const;
	//// Overload writes output to passed in pixelTransferBuffer
	//void toPixelTransferBuffer2(shared_ptr<GLPixelTransferBuffer>& buffer, const ImageFormat* outFormat = ImageFormat::AUTO(), int mipLevel = 0, CubeFace face = CubeFace::POS_X, bool runMapHooks = true) const;
