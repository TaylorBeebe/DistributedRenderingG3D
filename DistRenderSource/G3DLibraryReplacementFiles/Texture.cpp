...
static const shared_ptr<Framebuffer>& workingFramebuffer2() {
    static shared_ptr<Framebuffer> fbo = Framebuffer::create("Texture workingFramebuffer 2");
    return fbo;
}
...
shared_ptr<Texture> Texture::fromFile
   (const String&                   filenameSpec,
    ...
    if (dimension == DIM_2D_ARRAY) {
        ...
        return Texture::fromPixelTransferBuffer(FilePath::base(filenameSpec), Image::arrayToPixelTransferBuffer(images), desiredEncoding.format, dimension);
    }
    ...
    if ((toLower(loadingInfo->filename[0]) == "<whitecube>") || (toLower(loadingInfo->filename[0]) == "<white>") || loadingInfo->filename[0].empty()) {
        ...
        const shared_ptr<Texture>& instance =
            createShared<Texture>(FilePath::base(filenameSpec), first->width(), first->height(), first->depth(), 
    ...
    const shared_ptr<Texture>& instance = createShared<Texture>(FilePath::base(filenameSpec), width, height, depth, dimension, desiredEncoding, 1, true);
    instance->m_loadingInfo = loadingInfo;
    loadingInfo->preprocess = preprocess;
...
void Texture::toPixelTransferBuffer(shared_ptr<GLPixelTransferBuffer>& buffer, const ImageFormat* outFormat, int mipLevel, CubeFace face) const {
    debugAssertGLOk();
    force();
...
shared_ptr<Image> Texture::toImage(const ImageFormat* outFormat, int mipLevel, CubeFace face) const {
    return Image::fromPixelTransferBuffer1(toPixelTransferBuffer(outFormat, mipLevel, face),100,100);
}

shared_ptr<Image> Texture::toImage5(const ImageFormat* outFormat, int w, int h, int mipLevel, CubeFace face) const {
	return Image::fromPixelTransferBuffer1(toPixelTransferBuffer(outFormat, mipLevel, face),w,h);
}
