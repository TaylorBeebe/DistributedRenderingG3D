...
shared_ptr<Image> Image::fromPixelTransferBuffer1(const shared_ptr<PixelTransferBuffer>& buffer, int width, int height) {
	const shared_ptr<Image>& img = create(width, height, buffer->format());
	img->set(buffer,width,height);
	return img;
}
